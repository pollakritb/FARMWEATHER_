...
async function loadCurrent(force = false) { try { const data = await api(`/plots/${state.selectedPlotId}/weather/${force ? 'current/refresh' : 'current'}`, force ? { method: 'POST' } : {}); $('#temperature').textContent = number(data.temperatureC); $('#rain').textContent = number(data.rainfallMm); $('#humidity').textContent = number(data.relativeHumidityPct); $('#wind').textContent = number(data.windSpeedMs); const distance = data.source === 'TMD_FORECAST_FALLBACK' ? null : `ห่าง ${number(data.stationDistanceKm)} กม.`; $('#observation-meta').textContent = [data.stationName, distance, date(data.observedAt)].filter(Boolean).join(' · '); } catch (error) { $('#observation-meta').textContent = `โหลดไม่ได้: ${error.message}`; } }
